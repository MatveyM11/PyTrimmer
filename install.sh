...
if [ -t 0 ]; then
    ...
    TARGET_FILE="PyTrimmer.desktop"

    # Get the current working directory
    CURRENT_DIR=$(pwd)

    # Find the actual path of the PyTrimmer.py file
    PyTrimmer_QT_PATH=$(find "$CURRENT_DIR" -type f -name "pytrimmer.py" 2>/dev/null | head -n 1)

    # Find the actual path of the PyTrimmer.png file
    PyTrimmer_ICON_PATH=$(find "$CURRENT_DIR" -type f -name "PyTrimmer.png" 2>/dev/null | head -n 1)

    # Update the Exec and Icon lines in the target file with the actual paths
    sed -i "s|^Exec=.*|Exec=python3 \"${PyTrimmer_QT_PATH}\"|" "$TARGET_FILE"
    sed -i "s|^Icon=.*|Icon=${PyTrimmer_ICON_PATH}|" "$TARGET_FILE"
